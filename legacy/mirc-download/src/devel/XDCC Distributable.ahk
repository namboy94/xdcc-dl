...
mircdownload(){
	...
	loop %totalnumberofseries%{									;download loop
		...
		loop %mircpackloop%{									;pack enter loop
			...
			sleep 100
			send {enter}
			;FileCreateDir, %workdir%\Downloads\%showname%
...
guiactive(){
	...
	Gui, Add, Button, w200 h100 x30 gSingleP, Single Pack
	Gui, Add, Button, w200 h100 x30 gcreateLog, Create and open log
	...
	SingleP:
	channelparser()
	inputBox, singlepacker, Single Pack, Enter the pack
	channelchecker(singlepacker,1)
	mircprepare()
	
	send {raw}/server %ircserver1%
	send {enter}
	sleep 8000
	send {raw}/join #									
	send {raw}%channel1%
