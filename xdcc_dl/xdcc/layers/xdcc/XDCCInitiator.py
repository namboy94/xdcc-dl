...
class XDCCInitiator(MessageSender):
    ...
    def dcc_send_handler(self, ctcp_arguments: List[str],
        ...
        if os.path.exists(self.current_pack.get_filepath()) and \
            ...
            if position >= self.filesize:
                ...
                self.logger.log("File already completely downloaded. Aborting",
                ...
                self.logger.log("Requesting DCC RESUME",
    ...
    def dcc_accept_handler(self, ctcp_arguments: List[str],
        ...
        :return:               None
        """
        self.logger.log("DCC RESUME request successful",
                        LOG.DCC_RESUME_SUCCESS)
        self.logger.log(
            "Resuming Download of " + self.current_pack.get_filepath(),
            LOG.DOWNLOAD_RESUME
        )
    ...
    def on_privnotice(self, connection: irc.client.ServerConnection,
        ...
        try:
            if "You already requested that pack" in event.arguments[0]:
                self.logger.log(
                    "Pack already requested, waiting for next Ping",
                    LOG.ALREADY_REQUESTED
                )
                self.already_requested = True
            elif "You will have to re-send that," \
                 "to the bot that transferred the file." in event.arguments[0]:
                connection.privmsg(
                    self.current_pack.get_bot(),
                    self.current_pack.get_request_message()
                )
            elif "** XDCC SEND denied, you must be on a known channel" \
                 "to request a pack" in event.arguments[0]:
    ...
    def on_ping(self, connection: irc.client.ServerConnection,
        ...
        if self.already_requested:
            log_message = \
                "Resending XDCC Request to " + self.current_pack.get_bot()
            log_message += \
